...
class pithermalcam:
    ...
    def __init__(self,use_f:bool = True, filter_image:bool = False, image_width:int=800, 
                image_height:int=480, output_folder:str = '/home/pi/pithermalcam/saved_snapshots/'):
        self.use_f=not use_f
    ...
    def _setup_therm_cam(self):
        ...
        time.sleep(0.1)
        #setup contour camera
        self._video_feed = pvs.PiVideoStream().start()
        time.sleep(2.0)
    ...
    def change_video_res(width, height):
        self._video_feed.set(3, width)
        self._video_feed.set(4, height)
    ...
    def _pull_raw_image(self):
        ...
        # Pull one cam image / Grab a single frame of video
        self._raw_cam_image = self._video_feed.read()
...
#         #convert reg img to grey
#         img_grey = cv2.cvtColor(self._raw_cam_image,cv2.COLOR_BGR2GRAY)
#         #set a thresh
#         thresh = 20
#         #get threshold image
...
#         self._cam_image = np.zeros(self._raw_cam_image.shape)
#         # draw the contours on the empty image
#         cv2.drawContours(self._cam_image, contours, -1, (0,255,0), 3)
#         
        self._cam_image = self._raw_cam_image
        
        #add overlay with camera
        self._overlay_camera_feed()
    ...
    def _add_image_text(self):
        ...
        cv2.putText(self._image, text, (30, 18), cv2.FONT_HERSHEY_SIMPLEX, .8, (255, 255, 255), 1)
    ...
    def _overlay_camera_feed(self):
        # rotate the camera
        frame = cv2.rotate(self._cam_image, cv2.ROTATE_180)

        # Resize frame of video to the same size as the thermal image
        height, width, channels = self._image.shape
        #print("height: "+str(height)+" / width: "+str(width)+" / channels: "+str(channels)) 
        cam_frame = cv2.resize(frame, (width, height))

        #overlay these two frame together
        tr = 0.5 # transparency between 0-1, show camera if 0
        self._image = ((1-tr) * cam_frame.astype(np.float) + tr * self._image.astype(np.float)).astype(np.uint8)
